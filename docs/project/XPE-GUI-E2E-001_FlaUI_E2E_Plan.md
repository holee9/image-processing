...
> **게이트를 어떻게 읽는지 (leader, 2026-09-16 / GUI-C-68).** §4.1 과 §4.2 의 제목이 스위트별 벽시계 상한을 정합니다 — Smoke **< 30 s**, Workflow **< 3 min**. 지금까지 이 둘이 **어느 시간에 걸리는지 문서가 말하지 않아** 실제로 잘못 쓰였습니다. 아래가 그 정의입니다.
>
> **1. 각 게이트는 그 스위트에만 걸립니다.** Native 실행 전체나 다른 스위트를 여기 비교하지 마십시오. GUI-C-43 이후 다섯 카드 동안 §4.2 의 3 min 이 **Native 실행 전체**에 적용돼 왔고, 그 결과 **여유가 많은 쪽을 걱정하고 빠듯한 쪽을 한 번도 안 봤습니다** — 실측은 Workflow 26.4~27.1 s(게이트의 15%), Smoke 22.9~23.2 s(게이트의 **77%**)입니다.
>
> **2. 게이트는 스위트 자신의 일만 잽니다.** 프로세스당 1회인 준비 작업(XCal 교정 생성 등)은 **가장 먼저 만들어지는 픽스처에 붙으므로**, 그것을 포함해 세면 같은 코드가 **테스트 실행 순서에 따라 23 s 또는 61 s** 가 됩니다. 순서는 스위트의 성질이 아닙니다. **전체 벽시계는 두 경우 모두 같다는 것이 측정됐고**(약 90 s), 따라서 그 차이는 신호가 아니라 귀속의 잡음입니다. **1회 준비 비용은 따로 보고하고 게이트에 넣지 않습니다.**
>
> **3. 이 수치는 이 개발 기계 것입니다.** CI 러너에서는 다릅니다 — 같은 저장소에서 검출 성능이 로컬 702 ms 대 CI 1340 ms(**1.91배**)로 갈린 선례가 있습니다(`SPEC-XPE-P1A`). **CI 스위트별 시간은 아직 측정되지 않았습니다.**
>
> **부수 — leader 의 오류를 기록합니다.** 제가 이 게이트의 출처를 찾으려고 저장소에서 `180` 을 검색했고, 못 찾자 레인에게 **"180 s 게이트가 저장소 어디에도 없습니다"** 라고 단언했습니다. **틀렸습니다** — 문서가 `< 3min` 으로 적어서 숫자 검색에 안 걸렸을 뿐입니다. 레인이 "Gate" 라는 단어로 한 번에 찾았습니다. **부재를 주장할 때는 값이 어떻게 쓰였을지가 아니라 무엇을 뜻하는지로 찾아야 하고**, 못 찾았을 때는 "없다" 가 아니라 **"못 찾았다"** 라고 적어야 합니다.
...
> **측정 정정 (leader, 2026-09-16 / GUI-C-69) — 위 트레이트는 게이트를 걸지 않습니다.**
>
> 세 가지가 확인됐습니다. **`SmokeScenarios.cs` 에 `Category` 트레이트가 없습니다.** **`Category="Workflow"` 는 저장소 전체에 0건입니다.** 그리고 **게이트를 읽어 실패시키는 도구가 `ci.yml` 에 없습니다.**
>
> 세 번째가 가장 중요합니다 — **§4.1·§4.2 는 지금까지 관습이었지 기계적 게이트가 아니었습니다.** 두 번째도 같은 방향입니다: 트레이트로 §4.2 를 거는 도구가 나중에 생기면 **잴 대상이 비어 통과합니다.** 이 저장소가 반복해 만난 형태입니다 — 검사가 있으되 아무것도 안 보는 것(#157 · #160 · #161 · #165 · QA-B-64 의 왕복 검사).
>
> **판정 1 — 지금 정본은 폴더입니다.** 트레이트가 실재하지 않으므로 그것을 기준이라 적는 것은 사실이 아닙니다. 측정은 **클래스 경로(폴더)** 로 하고, 위 코드 블록은 **채택된 규약이 아니라 계획**으로 읽으십시오. 트레이트를 실제로 도입할지는 분류를 정한 뒤의 일이고, **먼저 붙이면 분류가 굳습니다.**
>
> **판정 2 — `< 30s` 는 근거가 없고, 재유도가 필요합니다.** 값의 유도가 이 문서에 없습니다. 다만 §4.1 표가 **다섯 건**이고 C-29 노트가 그 다섯을 **584 ms** 로 쟀으니 약 50배였습니다. **지금 그 자리에 21건이 들어 있습니다 — 게이트는 그대로인데 대상이 4배가 됐습니다.**
>
> **판정 3 — 이 게이트의 성격은 성능 게이트와 다릅니다.** `SPEC-XPE-P1A` 의 성능 게이트는 **고정된 일에 걸리므로 느려졌다고 올리면 안 됩니다.** 이 게이트는 **자라는 일에 걸립니다** — 시나리오가 늘어난 것은 의도된 것이고, 그때 숫자를 안 고치면 **게이트가 조용히 "테스트를 더 넣지 마라" 가 됩니다.** 그건 이 게이트가 하려던 일이 아닙니다.
>
> **따라서 재유도의 기준은 총합이 아니라 시나리오당 비용입니다.** 총합이 자라는 것은 정상이고, **한 건당 비용이 자라는 것은 신호**입니다. 재유도는 그 수치가 나온 뒤에 합니다 — 584 ms / 5 건과 지금의 25.5 s / 21 건은 **측정 조건이 같지 않아 그대로 나눌 수 없습니다**(앱 기동 포함 여부가 확인되지 않았습니다).
>
> **지금 조치는 없습니다.** CI 실측이 Mock 25.5 s(85%) · Native 22.9 s(76%) 로 **게이트 안**입니다(런 `35071370089`). 넘기 전에 정하는 것이 목적이었고, 넘지 않았으므로 재유도의 입력을 먼저 모읍니다.
