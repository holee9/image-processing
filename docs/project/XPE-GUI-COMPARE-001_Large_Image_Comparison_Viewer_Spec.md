...
- Detached viewer 상태 모델 재사용 — **부분**(아래 정정 참조)

> **정정 2026-09-16 (leader, GUI-C-70~C-72 / #166) — 이 줄이 "완료" 였고, 그 시점에 창이 아예 열리지 않았습니다.**
>
> `Detach Comparison Viewer` 를 누르면 `BindDetachedViewport` 의 **첫 바인딩에서 예외**가 났고(`SourceImage` 는 setter 가 private 인데 `TwoWay` 로 걸림), **`Window` 는 그 뒤에 만들어지므로 생성되지 않았습니다.** 그리고 **아무 신호도 없었습니다** — 성공 로그가 `Show()` 뒤에 있어 도달하지 않았고, 상태 표시줄·대화상자도 없었습니다.
>
> **"완료" 가 언제 적혔고 무엇을 근거로 했는지는 확인하지 않았습니다.** 확실한 것은 **2026-09-16 측정 시점에 그 문장이 거짓이었다**는 것뿐입니다.
>
> **지금 상태 — 셋으로 갈립니다.**
>
> | 성질 | 상태 |
> |---|---|
> | 창이 열린다 | **확인됨** (GUI-C-71, `#166` 닫음) |
> | 창 → 본체 되돌려 쓰기 | **부분 확인** — `ZoomScale` 하나를 실행으로 관측(분리 창에서 휠 → 메인 확대율 변화). 나머지 다섯은 **"바인딩이 거부되지 않는다" 까지** |
> | 본체 → 창 영상 반영 | **관측점 없음** — 아래 |
>
> **세 번째가 `GUI-CMP-FR-004` 의 본체입니다**("unsynchronized viewer 를 만들지 않는다"). 그리고 **지금 그것을 볼 도구가 없습니다.** UIA 는 분리 창에서 요소 7개만 내놓고 여섯이 타이틀바 크롬이며, 남은 하나는 뷰모델에 바인딩된 상태 줄이라 **뷰포트가 따라가든 말든 같은 값을 읽습니다**(뷰포트는 `FrameworkElement` 라 peer 가 없습니다). 픽셀 캡처는 분리 창이 하얗게 나오는데, **같은 방법으로 메인 창을 찍어도 하얗습니다** — 그 캡처는 이 앱의 WPF 내용을 못 보는 도구이고, **하얀 사진은 분리 창에 대해 아무 말도 하지 않습니다.**
>
> **`GUI-CMP-VER-005`("Detached-window automation") 는 두 시나리오로 일부 충족됩니다** — 창이 열리는지, 그리고 창에서 본체로 상태가 흐르는지. **영상 방향은 자동화로 덮이지 않으며, 그 문서가 대안으로 적은 "manual UAT checklist" 가 현재 유일한 경로**입니다.
>
> **이 줄을 다시 "완료" 로 바꾸려면 세 성질이 모두 관측돼야 합니다.**
