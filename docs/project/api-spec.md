...
## 0. Export inventory (machine-derived, normative)

**Generated 2026-09-12 from the headers, not from prose.** QA-B-54 measured the six
post-owned modules and found the headers and the built DLLs agree on all 45 exports, while
the narrative sections of this document disagreed with both: 8 signature mismatches, **10
functions that exist nowhere**, and whole modules missing (the display section and the real
`xpe_display` exports had an **empty intersection** — only the count matched). Three separate
corrections in this document had themselves introduced errors, because a correction applies
where it is written and leaves the neighbouring sentence making the same claim untouched.

So this table, not the prose below, is the inventory. It is regenerated by scanning
`modules/*/include/**/*.h` for `XPE_API` / `GSVG_API` declarations — the same source the
compiler reads. Where a later section contradicts this table, **this table is correct and
the section is stale**; sections are corrected as they are touched rather than in one sweep,
because a sweep is how the previous errors were introduced.
...
the test suites are for; the second is #142 work in progress.
...
**Description**: Combines `num_frames` dark-field frames into an offset calibration map and writes it as XCal to `output_path` (with `integration_time_ms` / `temperature_c` recorded in the header). `config_json_or_null` selects the per-pixel combination method per XPE-ALG-001 §9.8 (`{"method":"mean"|"sigma_clip"|"median"|…, "kappa":3.0, "max_iter":5}` — keys as accepted by the module's offset-generation config parser); `NULL` keeps the default (`mean`), so existing callers are unchanged. With `sigma_clip`, pixels whose surviving sample count falls below `N_min = max(3, ⌊N/4⌋)` are marked as static defects and OR-merged into the module-global defect map (§9.8.2.1, decision #138 / QA-A-38); the offset value itself stays the clipped mean (§9.8.3). If at least one pixel is marked and a defect map with a different width or height is already loaded, the call returns `XPE_ERR_CONFIG_INVALID`: the marks are not merged, the loaded map is left unchanged, and `output_path` is not written (documented 2026-09-17, QA-A-93; behaviour unchanged). **Corrected 2026-09-11 (QA-A-38/A-39):** this section previously documented a `(frames, frameCount, offsetMapOut, configJsonOrNull)` form that never existed in the header; the six-argument form is canonical. Before QA-A-39 the public entry point had no config argument and always used `mean`, so `sigma_clip` was unreachable from the public API.
**SRS**: SRS-CALIB-020  
**Thread safety**: Reentrant.  
