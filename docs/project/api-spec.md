...
**Description**: Replaces defective pixels with an edge-aware bilinear interpolation over a 5×5 neighbourhood excluding the centre, using the defect map (BPM) held in the module-global calibration store (loaded beforehand by `xpe_calib_load_defect_map`). Input and output are `XPE_PIXEL_FLOAT32`; `output` must be a caller-allocated buffer of the same dimensions as `input`, distinct from `input`. `metadata` is required and supplies the dose-dependent threshold for runtime transient-defect detection; the static BPM takes priority over runtime detection. There is no `configJsonOrNull` parameter on this function — interpolation behaviour is fixed and configured at the pipeline level.  
