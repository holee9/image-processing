...
> **`step_size` 정정 (2026-09-19, QA-B-139, #162).** **이 표에서 `step_size` 를 뺐습니다.**
> 위의 2026-09-17 실측 주석(QA-B-78)은 *"연결할지 제거할지 정해지지 않았다"* 로 끝났고,
> **QA-B-139 가 코드에 물어 답을 얻었습니다 — 이을 상대가 없습니다.**
>
> GL 보폭 `h` 는 헤더 주석에만 있고, `computeFractionalMask` 는 계수만 만들며 합성곱은
> **정수 화소 간격 샘플링**입니다(보간 없음). 미분 기여를 조절하는 유일한 배율
> `gain = min(1 + 0.5·order, 2.0)` 은 **`order` 에서 유도**되고, 그 상한의 이유가
> **SAF-100 의 클리핑 여유분**으로 주석에 명시돼 있습니다. 반복은 전량 적용 N회라
> **이완 계수가 들어갈 자리도 없습니다.**
>
> 즉 `step_size` 를 잇는 것은 배선이 아니라 **세 개의 새 설계 결정**이고, 하나는
> **안전 요구를 건드립니다.** 따라서 잇지 않고, **값이 살아 있는 척하던 배선만
> 걷어냈습니다** — 파싱, `[0.01, 1.0]` 클램프, `XPE_FRAC_DEFAULT_STEP`, 그리고
> 로그의 `step={:.2f}`(로그 독자가 보폭이 적용됐다고 읽는 줄).
>
> **키 이름은 `kKnown` 에 남겨 두었습니다.** 빼면 *"모르는 키"* 경고가 나는데, 그건
> **틀린 설명**입니다 — 키는 알려져 있고 **효과가 정의되지 않은** 것입니다. 호출자는
> 무효 키 경고로 이미 신호를 받습니다.
>
> 이 문서를 보고 `step_size` 를 쓰던 호출자가 있다면, **그 값은 이전에도 아무것도 하지
> 않았습니다**(`maxdiff = 0.000000000`, 대조군 `iterations` 1 대 3 은 494.17).
