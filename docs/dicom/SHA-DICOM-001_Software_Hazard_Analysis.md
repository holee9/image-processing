...
**통제 범위 — 실측 (2026-09-12, QA-B-49 ~ B-52).** 위 통제가 실제로 어디까지 막는지를 실행으로 확인했다. 통제표만 읽으면 "손상 파일은 거부된다" 로 읽히지만, 검출되는 손상과 그렇지 않은 손상이 갈린다.

| 손상 유형 | 검출 | 근거 |
|---|---|---|
| 구조 손상 (preamble · VR · 길이 필드) | **검출** | DCMTK parser (C-002-A/B) |
| 메타 그룹 부재 · SOP/TS 불일치 | **검출** | `xpe_dicom_validate` (§11.11) |
| 비압축 픽셀이 선언보다 **짧음** | **검출** (2026-09-12부터) | QA-B-49 `6c9356b` — 그 전에는 `XPE_OK` + 검정 패딩 |
| 압축 산출 치수가 선언과 **다름**(양방향) | **검출** (2026-09-12부터) | QA-B-50 `7de6e16` · QA-B-51 `5f35268` — J2K 는 선언을 아예 안 봤고, JPEG-LL 은 DCMTK 가 패딩해 앞 가드가 못 봤다 |
| **값이 손상됐고 길이는 맞음** | **검출 안 됨** | QA-B-50 §3 — `modules/dicom/src/` 에 CRC · 체크섬 · 해시 **grep 0건**. 압축 경로에서 걸리는 것은 디코더가 우연히 실패해 주는 경우뿐이고, 디코드에 성공하는 손상은 통과한다 |

**마지막 행이 이 통제의 한계다.** DICOM 표준은 픽셀 데이터 체크섬을 요구하지 않으므로 이 계층에서 막을 수단이 없다 — 통제는 저장·전송 계층(파일시스템 무결성, 전송 프로토콜의 검사합)에 있어야 한다. 잔존 위험도 평가는 그 전제 위에서 읽어야 하며, 그 계층의 통제가 없는 배치에서는 이 값이 성립하지 않는다.

**C-002-D 의 실제 형태**: 부분 데이터 금지는 이제 `XPE_ERR_DICOM_INVALID` 반환 + `outImg` 를 `data=NULL` · `dataSize=0` 으로 두는 것으로 구현된다(QA-B-48 이 실패 경로의 출력 불변을 센티널로 확인했고, 치수 판정은 alloc 이전이라 그 계약이 성립한다).
