...
> **[미해결 — #171] 2026-09-17 측정.** 아래 HAZ-GUI-004 의 잔여 위험 **Low** 는 근거가 없습니다.
> - 통제 `IsPreviewStale` 표시기는 **코드에 존재하지 않습니다** (`gui/`·`clients/` 0건, 대조군 `.Freeze()` 5건)
> - 검증 **W-06 은 시험 스위트에 존재하지 않습니다** (대조군 W-07 은 존재하나 오래된 미리보기를 보지 않음)
> - ~~통제 (3) 에 해당하는 코드(`MainWindowViewModel.cs:866`)는 새 결과가 없을 때 이전 영상을 유지한다~~ — **이 판독은 틀렸습니다**(866행은 null 이 아님을 확인한 분기 안). **그러나 위해 상황은 다른 경로로 실행 재현됐습니다**: VOI 입력란을 고치면 영상이 갱신되지 않고, 영상 옆 HUD 는 **그 영상을 만들지 않은 값**을 표시합니다 (GUI-C-77, #171)
>
> **등급은 고치지 않았습니다.** 통제를 구현할지, 통제 없이 위험을 다시 평가할지는 위험 관리 결정이며
> #171 에서 다룹니다. 이 표시는 그 결정이 날 때까지 **아래 등급을 인용하지 말라**는 뜻입니다.
