...
> **[미해결 — #175] 2026-09-17 측정. 아래 잔여 위험 Low 는 근거가 없습니다.** 인용하지 마십시오.
>
> | 통제 | 실측 (범위: `gui/`·`clients/` 의 cs·xaml) |
> |---|---|
> | (1) Mock 모드 시 해제 불가 경고 배너 | **0건** |
> | (2) 타이틀바 `[MOCK]` | **0건** — 창 제목은 고정값 `"ImageProcTest GUI-S0"` |
> | (3) 내보내기 보고서에 Mock 표기 | 미확인 |
> | (4) 운영 GUI 는 Mock 폴백 금지(hard fail) | 미확인 — 운영/시험 모드 구분이 코드에 있는지부터 |
> | 검증 E-01 `DllMissing_ShowsAlert` | **0건** — 시험이 존재하지 않음 |
>
> 대조군(같은 검색): `IsPreviewStale` 4건, `XPE-SKIP-ALLOWED` 1건.
>
> **위해 상황은 관측됐습니다** (GUI-C-81): Native 요청 후 DLL 로드에 실패하면 팩토리가 **조용히 Mock 으로
> 떨어지고**, 상태 표시줄은 **요청값 `mode=Native`** 를 계속 표시합니다(`MainWindowViewModel.cs:447`).
> 그리고 알림은 수집되지만 **화면 어디에도 표시되지 않습니다**(GUI-C-76) — 위 "Sequence of Events" 의
> "UI 에 경고 미표시" 가 그대로입니다.
>
> 이것은 HAZ-GUI-004(`#171`)와 **같은 형태**입니다 — 잔여 위험 등급이 존재하지 않는 통제와 존재하지 않는
> 검증 위에 서 있습니다. 심각도는 운영 환경에서 **Critical** 로 더 높습니다.
