...
> **[#171] 통제 구현·검증 완료, 잔여 위험 Low 유지 (사용자 결정 2026-09-17).**
>
> 이 위해에 대해 한때 "통제가 코드에 없고, 검증 시험 W-06 도 없다" 는 것이 측정됐고(아래 이력),
> 위해 상황이 **실행으로 재현**됐습니다 — VOI 입력 시 영상은 갱신되지 않고 영상 옆 HUD 는 그 영상을 만들지
> 않은 값을 표시했습니다. 사용자 결정(원천 제거 + 라벨 정정 + 표시기)에 따라 다음이 구현됐습니다.
>
> | 통제 | 구현 | 검증 (E2E) |
> |---|---|---|
> | ② HUD 를 **그 영상을 실제로 만든 값**에 바인딩 | 메인 창·분리 뷰어 | W-21, W-25 |
> | ① VOI 를 적용 없이 고치면 **오래됨 표시** (즉시 재렌더는 취소·순서 보장이 없어 새 오래된 영상 경로를 만들므로 채택하지 않음) | 메인 창·분리 뷰어 | W-22, W-25 |
> | ③ 렌더 실패 시 `IsPreviewStale` **오래됨 표시** | 메인 창·분리 뷰어 | W-23, W-26 |
> | (선행) 메인 뷰포트가 영상을 실제로 받음 (`#172`) | 바인딩 복원 | W-20 |
> | 결함 주입 장치가 일반 실행에서 비활성 | 명령줄 전용 | W-24 |
>
> **각 시험은 해당 통제만 끄면 그것만 실패**하는 것으로 반증됐습니다. Mock 과 **Native** 양쪽에서 통과했고,
> Native 시험은 실제 백엔드를 거쳤다는 것(`src=`)을 단언합니다 — 이 단언이 없으면 Native 가 조용히 Mock 으로
> 떨어져도 통과했습니다(`#175`). CI `3537a13` 의 `gui-e2e-native` 초록.
>
> **이전 표시의 "W-06" 은 존재하지 않았습니다.** 검증 추적은 위 표의 W-20~26 으로 대체됩니다.
>
> **잔여 위험 Low 유지 (사용자 결정, 2026-09-17).** 원래 분석은 오래됨 표시기를 전제로 Low 를 줬고, 그 전제가
> 구현·검증됐으며, 원래보다 통제가 둘 더 있습니다(라벨 정정, 적용 전 표시).
>
> **수용된 잔여 위험**: **전처리 거절 경로**는 ③ 의 범위 밖입니다 — 전처리가 거절되면 이전 영상이 오래됨 표시
> 없이 남을 수 있습니다. 이 경로를 잔여 위험으로 수용하고 Low 를 유지합니다.
>
> <details><summary>이력: 2026-09-17 최초 측정 시점의 표시</summary>
>
> 통제 `IsPreviewStale` 0건, W-06 0건, `:866` 판독(틀린 판독이었음 — 그 행은 죽은 코드) 등. 전문은 `#171` 본문.
> </details>
...
> **[#175] 통제 구현·검증 완료, 잔여 위험 Low 유지 (2026-09-17, 사용자 결정 조건 충족 — 통제 (3) 파일 실측 뒤 종결).**
>
> 한때 아래 통제가 **코드에 하나도 없었고** 검증 시험 E-01 도 존재하지 않았습니다. 위해 상황도 관측됐습니다 —
> Native 요청 후 DLL 로드에 실패하면 조용히 Mock 으로 떨어지는데 상태 표시줄은 요청값 `mode=Native` 를 표시했습니다.
>
> | 통제 | 구현 | 검증 (E2E) |
> |---|---|---|
> | 상태 표시줄이 **실제** 백엔드를 표시 (`mode=Mock (requested Native)`) | 원천 하나(`RuntimeInfo`) | E-01a |
> | (1) Mock 일 때 **해제 불가 경고 배너** | 메인 창·분리 뷰어 | E-01b |
> | (2) 제목 **`[MOCK]`** | 메인 창·분리 뷰어 | E-01c |
> | (3) 보고서 3종에 실제 모드 기록 | menu-command-report, automation report, 증거 묶음의 `backend.json` — 셋 다 실제·요청 모드를 함께 기록 | GUI-C-85 파일 실측(아래) |
> | (4) 운영 GUI 는 Mock 폴백 금지 | **적용 대상 없음** — 운영/시험 모드 구분이 코드에 없음 | — |
> | **대조군**: Native 정상이면 배너·`[MOCK]` 없음 | — | E-01d — "항상 켜진 배너" 구현을 잡음 |
>
> 각 시험은 해당 통제만 끄면 그것만 실패합니다(반증, Native). CI `d0737d8` 의 `gui-e2e-native` 초록.
> **의도적 Mock(요청도 Mock)에도 배너와 `[MOCK]` 이 뜹니다** — 이 위해의 문언이 "Mock mode 시" 이기 때문입니다.
>
> **자동화 보고서 판정 (GUI-C-83, 완료)**: 요청 모드와 실제 모드가 다르면 `Passed=False` 입니다. 의도적 Mock(요청도
> Mock)은 통과합니다. GUI-C-84 부터 `Passed=False` 면 종료 코드도 1 입니다. CI `8976f73` 의 `gui-e2e-native` 초록.
> 배너의 해제 불가는 구성으로만 보장됩니다(자동화 트리에 닫기 컨트롤이 노출되지 않음).
>
> **통제 (3) 실측 (GUI-C-85)**: 요청/실제 = Native/Native, Native/Mock(대체), Mock/Mock 세 경우에서 세 보고서의 모드 필드를
> 파일로 읽었습니다. 세 보고서 모두 경우 a 와 b 의 실제 모드 값이 다르고(상수 아님), b 와 c 는 요청 모드 필드로 구별됩니다.
> 자동화 종료 코드는 0 / 1 / 0 입니다. 사용자 결정 조건이 충족되어 Low 를 유지하고 `#175` 를 닫습니다.
>
> **받아들인 잔여 위험과 별건**: 증거 **묶음(zip)** 내보내기는 `RunId` 가 설정되지 않아 모든 경우에 실패합니다(상태줄에 실패 표시).
> 같은 이유로 모든 실행이 한 `evidence/` 폴더를 덮어써, 지난 실행의 모드 기록이 남지 않습니다. 기록 내용은 맞으므로 이 위해의
> 오인 경로는 아니며, `#178` 로 분리했습니다.
>
> <details><summary>이력: 2026-09-17 최초 측정 시점의 표시</summary>
>
> 배너 0건, `[MOCK]` 0건(창 제목 고정값), 검증 E-01 0건. 전문은 `#175`.
> </details>
