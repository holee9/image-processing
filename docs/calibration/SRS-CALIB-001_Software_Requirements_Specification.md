...
| **SRS-CALIB-FUNC-027** | System shall support multi-point gain polynomial fitting via `xpe_calib_generate_gain_polynomial()`. Input: array of gain maps at N ≥ 3 dose levels (each from FUNC-026), dose levels array. Algorithm: For each pixel (x,y), fit polynomial `G(x,y,D) = Σ(c_k × D^k)` where `D` is the **detector dose (signal level)** and k=0..degree-1 (degree ≤ 4). Use least-squares regression. Output: polynomial coefficient array stored in `.xpe_calib` with `XCAL_TYPE_GAIN_POLY`. Coefficient validation: all coefficients must be finite; polynomial must be monotone in [D_min, D_max]. If non-monotone, reduce degree until monotonicity achieved (minimum degree 1 = linear). | Detector response is non-linear in dose, and the non-linearity differs from pixel to pixel: a single gain map taken at one dose level leaves residual per-pixel error at other exposures. A per-pixel polynomial over N dose levels captures that. CalData_6 (6 dose levels) and cyan_test (5 CalSet levels) provide real calibration data for validation. **Beam quality (kVp) is held fixed while dose is varied** — energy dependence is a separate concern handled by per-kVp gain maps, not by this polynomial. | Test: CalData_6 polynomial fit, cyan_test CalSet verification, monotonicity check, degree fallback |

> **요구 개정 (2026-09-18, #187 — 리더 결정).** 이 요구는 한 항목 안에서 변수를 두 가지로 쓰고 있었습니다 —
> 입력은 "dose levels array", 모델 식은 `G(x,y,E)`, 단조 구간은 `[E_min, E_max]`, 근거란은 전부 kVp·에너지였습니다.
> **선량(신호 준위)으로 확정합니다.** 근거 넷이 독립적으로 같은 곳을 가리킵니다.
>
> 1. **구현이 선량입니다**(1차, 직접 확인). `xpe_calib_generate_gain.cpp:438` 이
>    `fit_polynomial_ls(dose_levels, y_vals, ...)` 로 X 축에 선량을 넣고, 단조성도
>    `[dose_min, dose_max]` 에서 검사합니다. API 주석(`preprocess_api.h:395`)은
>    `dose_levels` 를 **"mGy or relative units"** 로 적습니다 — kVp 단위가 아닙니다.
> 2. **검증 자료가 선량입니다**(1차, 직접 확인). `tests/test_data/cyan_test/README.md` §4.2 의 피팅
>    절차가 `X = [14037, 17285, 20985, 30868, 42677] (선량, ADU)` 입니다. kVp 라면 50–120 이어야 합니다.
> 3. **이 요구가 인용한 벤더가 선량을 말합니다.** 근거란이 인용한 Rayence 의 교정 매뉴얼은
>    bright frame 을 "8개 이상의 서로 다른 **노출 준위**" 로 취득하라고 적습니다. 인용된 "8 points" 는
>    8개 kVp 가 아니라 8개 노출 준위였습니다.
> 4. **업계는 에너지를 다항식으로 풀지 않습니다.** 다중 점 다항식은 선량 비선형용이고
>    (저선량 구간 1차식 + 고선량 구간 다항식, "기존 빔 조건 고정" 상태에서 선량만 변경),
>    kVp 의존은 **대표 kVp 별 개별 게인 맵**으로 처리하는 것이 관행입니다.
>
> **FUNC-006 과의 관계는 중복이 아니라 계층입니다.** FUNC-006 은 전역 LUT(모든 화소 공통 응답),
> 이 요구는 화소별 다항식 — 전역으로 못 잡는 화소별 편차를 잡습니다.
>
> **미확인**: Schmidgunst 2007 원문(Medical Physics, 유료)을 읽지 못했습니다. 서지 사항만 확인했고
> 초록도 보지 못했습니다. 3·4번 근거는 검색 색인의 인용문 수준(2차 자료)입니다. 1·2번은 저장소
> 안에서 직접 확인한 1차 근거입니다.
>
> **남은 문제**: 적용 시점에 선량 D 를 어디서 얻을지가 정해지지 않았습니다. kVp·mAs·SID 만으로는
> 유도할 수 없습니다(튜브 출력 계수·여과·피사체 감쇠가 빠짐). 후보는 영상 자체의 신호 평균
> (cyan_test 가 쓰는 방식) 또는 AEC 노출 지표이며, 별도로 정합니다.
