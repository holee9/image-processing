...
> **무결성 검사 정정 (2026-09-19, #188 — QA-A-106·QA-A-130)**
>
> **이 문서가 적는 CRC-32 서술(§SWU-1.5, §3.1.5, 의존성 표, 위험 표)은 옛 서술입니다.**
> 실제 무결성 값은 **SHA-256 32바이트**(헤더 오프셋 120, `static_assert` 로 고정)이고
> 헤더는 152바이트입니다. 실제 `.xcal` 파일 바이트로 확인했습니다.
>
> 따라서 아래 *"CRC-32 collision risk ~1 in 4 billion"* 행의 **위험 평가는 대상이 없습니다** —
> 쓰이는 알고리즘이 CRC-32 가 아닙니다.
>
> SSOT: `SRS-CALIB-001` §2.2 의 정정 블록. 무결성을 CRC-32 로 약화하지 않습니다.
