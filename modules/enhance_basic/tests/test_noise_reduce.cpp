...
// ---------------------------------------------------------------------------
// REQ-ENH-012 names no mode. Bilateral is measured (the existing proxy uses it);
// NLM is not -- its naive search at 3072x3072 with the default 21/7 windows is
// about 3072^2 * 21^2 * 7^2 ~ 2e11 inner steps, far beyond a benchmark run.
TEST(NoiseReduce, BenchmarkFreeze_Performance_REQ_ENH_012_Bilateral3072) {
    ...
    params.sigma_space = 3.0f;
    params.sigma_range = 50.0f;
    perf_measure::Measure("REQ-ENH-012/bilateral", "3072x3072", reset,
                          [&] { return xpe_noise_reduce(&img, &params); });
    for (size_t i = 0; i < n; ++i) {
