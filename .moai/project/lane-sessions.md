...
## 3.5 기록·이력 규약 (사용자 승인 2026-09-17)

**이 절이 없어서 생긴 문제였습니다.** 소유 경계(§1)는 *파일*만 정했고, **누가 이슈를 열고
누가 이력을 남기는가**는 어디에도 없었습니다. 관행으로 "리더가 다 한다" 가 되었고, 그
결과가 **열린 이슈 23건 중 10건 댓글 0** 입니다.

### 3.5.1 이슈 생성 — 리더

이슈는 리더가 엽니다. 레인은 보고서로 보고하고, 리더가 판정한 뒤 이슈로 옮깁니다.
레인이 이슈를 직접 열지 않는 이유는 §3.5.2 와 같습니다.

### 3.5.2 이슈 코멘트 — **레인은 사실, 리더는 판정** (둘 다)

| 주체 | 시점 | 무엇을 |
|---|---|---|
| **레인** | 카드 완료 시 | **사실만** — 실행한 명령, 나온 수치, 증거 파일 경로. **해석·판정 문장 금지** |
| **리더** | 판정 시 | 결론과 근거. 무엇이 확정됐고 무엇이 아직 아닌가 |

**왜 레인은 사실만인가 — 측정된 근거가 있습니다.** 세 레인이 각자 자기 사례를 셌습니다:

- **Lane A**: 최근 5카드에서 자기 결론과 최종 판정이 갈린 자리 **3건**(A-77 삭제 범위,
  A-78 "놓치신 곳이 아닙니다"가 절반 틀림, A-79 원인이 리더 푸시 타이밍)
- **Lane B**: `sensitivity` 를 "무반응" 으로 적었으면 틀렸을 것 — 실제로는 *이름과 반대로
  일하는* 것이었고 픽스처가 활동 구간 밖이었을 뿐. 그리고 `cleanup()` 거짓 음성을 믿었다면
  **이미 옳은 판정을 반박하는 코멘트**가 #147 에 달렸을 것
- **Lane C**: C-73 에서 `{0,0}` 을 "제품 버그" 로 적었으면 지금 틀린 문장이 서 있을 것 —
  실제 답은 "배치 코드가 아예 없다"

**비대칭이 핵심입니다.** 보고서는 뒤 보고서가 앞을 갱신하는 **연속 기록**이라 뒤집혀도
정상 작동이지만, **이슈 코멘트는 최신 사실로 읽히고 인용 가능합니다.** 사실만 적으면
나중에 뒤집혀도 틀린 기록이 되지 않습니다 — "한 픽스처에서 0.0/1.0 동일, 단언 보류" 는
결론이 바뀌어도 참입니다.

### 3.5.3 증거는 로컬, 결론은 이슈

`.moai/reports/` 와 `.moai/lanes/` 는 **`.gitignore:297`** 로 커밋되지 않습니다.
보고서 **224건이 git 추적 0건**입니다(pre 80 / post 69 / gui 75).

**그대로 둡니다.** 보고서는 대부분 *왜 그렇게 쟀는가* 이고(임계값 근거, 픽스처 설계,
반증 설계), 컴파일러 원문·로그 바이트까지 이슈에 붙이면 읽히지 않습니다.

**대신 이슈 코멘트가 재측정에 필요한 것을 담습니다**: 실행한 명령, 나온 수치, 증거 경로.
워크트리가 사라져도 **무엇을 재면 되는지**는 남습니다.

> **이 규약이 없어서 실제로 잃을 뻔한 것**: 창 재획득 결함은 Lane C 보고서 **11장**에 걸쳐
> 좁혀졌는데(빈도 7%, 대조군 50/50, 원인 후보 4개 배제) **열린 이슈가 없었습니다.**
> 2026-09-17 에 `#170` 으로 옮겼습니다.

### 3.5.4 표기 규칙 — `#` 는 이슈 번호에만

`#30011` 은 **UIA 오류 코드**인데 카드·보고서에서 `#` 를 붙여 쓴 탓에 이슈 번호로 읽혔고,
**CI 게이트의 allow-list 가 실제로 그렇게 키잉됐습니다**(2026-09-17 정정).

- 이슈: `#170`
- 오류 코드·식별자: `UIA 30011`, `C2143`, `EXS_JPEGProcess14SV1` — **`#` 를 붙이지 않습니다**

### 3.5.5 문서 갱신 — 소유자가, 다만 "수치는 적지 않는다"

문서는 §1 소유 경계를 따릅니다(리더: `docs/`·README·루트, 레인: 자기 모듈·`gui/`·`clients/`).

**그러나 갱신 주체보다 먼저 정할 것이 있습니다**: **측정값을 산문에 적으면 적는 순간
낡습니다.** 실제 사례 셋이 같은 계열입니다 —

| 문서 | 적힌 값 | 실제 |
|---|---|---|
| `modules/preprocess/docs/test_suite_report.md` | `Total Tests: 54` | ctest **643** |
| `README.md` | `A ~93 / B ~85 (2026-05-09 실측)` | 이후 재측정 없음 |
| (삭제됨) `simd_dispatch.cpp` 커밋 메시지 | `405/405 checks passing` | 하네스가 존재한 적 없음 |

**규칙**: 문서는 **어디서 측정하는지**를 가리키고(ctest, CI 워크플로, 벤치마크), 그 시점의
수치를 본문에 얼려 두지 않습니다. 수치를 적어야 하면 **측정일과 명령을 함께** 적습니다.
...
먼저 /rename leader 실행 후, ListAgents 로 레인 세션 3개 접속 확인.
