...
**Issue B1 (CRITICAL)**: api-spec.md §4 Summary table lists "xpe_common.dll = 15" but SPEC-XPE-MASTER v2.0.0 §P0-05 states "**18 API** (기존 15 + AED 3)".  
**Functions missing from api-spec §5.1~5.15**: 
- `xpe_aed_configure` (AED 1/3)
- `xpe_aed_poll_event` (AED 2/3)
- `xpe_aed_get_status` (AED 3/3)

**Status**: **RESOLVED** — api-spec.md v1.2.0 adds §5.16~5.18 with full AED function documentation. §4 Summary table updated to xpe_common=18.
...
2. **Document 3 AED functions** → Missing xpe_aed_configure/poll_event/get_status API contracts.  
...
- [ ] Update api-spec.md to v1.2.0 (add AED 3 functions, move EI function, update counts)
