...
> **장면 정의 보강 (2026-09-17, #183, QA-B-100)**
>
> "콜리메이션 경계" 는 **조사야 경계**를 뜻합니다. 조사야 안은 신호가 있고, 바깥은 산란만 있는 낮은 신호입니다. 균일한 배경 위에 선만 그린 장면은 조사야가 아니므로 이 기준의 시험 장면으로 쓰지 않습니다.
>
> 구현은 후보 사각형을 **로그 영역에서 본 안−밖 평균 차**로 고릅니다(X선 감쇠가 곱셈이기 때문). 1 DN 미만의 값은 1 DN 으로 올려서 계산합니다.
>
> **한계** — 영상만으로는 다음 경우를 풀 수 없습니다. 이때는 호출자가 조사야 마스크를 넘깁니다(`xpe_gsvg_process_masked`, #180).
> - 대비가 뒤집힌 변(안쪽이 바깥보다 어두운 변)
> - 폭이 넓은 밝은 띠
> - 강한 내부 경계가 16개를 넘는 경우
>
> 위 본문의 경계 위치(100/900/150/850)는 예시입니다. 실제 시험은 각자 정한 경계 위치에서 ±3 화소 허용을 확인합니다.
