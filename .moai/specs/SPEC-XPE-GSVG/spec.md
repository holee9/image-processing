...
> **두 층으로 나누어 읽으십시오 (2026-09-19, QA-B-135)**
>
> 위 1.008 은 **가상 그리드가 그 MC 안의 진짜 산란을 얼마나 잘 빼는가**입니다. 그 MC 가
> 현실과 얼마나 맞는가는 **별개의 층**이고, 이미 측정돼 있습니다(QA-A-95).
>
> | 층 | 묻는 것 | 값 | 출처 |
> |---|---|---|---|
> | ① 추정의 정확도 | 가상 그리드 추정 / 그 MC 안의 진짜 산란 | **1.008** (p05 0.964, p95 1.046) | QA-B-125 |
> | ② 산란장 자체의 타당성 | 우리 MC 의 SPR / 대외 공개값 | **0.56–0.65배**(Fetterly 2007, Solid Water 10 cm, 104 kVp, SPR 2.55)<br>**0.76–0.93배**(IAEA 표 6.1, PMMA 20 cm, 80 kV, SPR 5.93) | QA-A-95 |
>
> **두 층은 곱해집니다.** 추정이 1.008 로 정확해도 산란장 자체가 0.56–0.93배면 **실제로
> 빼는 양은 그만큼 부족합니다.** ①만 보고 "산란 보정이 정확하다" 로 읽으면 안 됩니다.
>
> 두 대외 비교는 **다른 출처이고 같은 방향**(우리가 낮음)이며, QA-A-95 의 민감도 실험에서
> **어느 변수 하나도 0.56배를 혼자 설명하지 못했습니다.**
>
> **[한계 — QA-A-95 가 적어 둔 것]**
> - **Fetterly 원문 미확인**(초록만). SID·공기 간격·여과·검출기·빔 스톱 크기를 모릅니다.
> - **IAEA 표 6.1 의 계산 방법·수상체 미상** — 측정인지 계산인지 표에 표시가 없습니다.
> - **조건이 우리와 다릅니다** — 두께·kVp·재질 전부. 조건이 다르면 자릿수 일치조차 약한
>   말입니다.
> - **모델에 없는 것**: 테이블·커버·격자 틀, 초점 밖 방사선, 검출기 후방 산란. 빔 스톱
>   측정은 이것들을 산란으로 셀 수 있으므로 **0.56배의 일부가 여기서 올 수 있습니다.**
>
> **배율은 넣지 않습니다.** 실장비 없이 문헌값에 맞추면 맞춘 것이 아니라 **가린** 것입니다
> (QA-A-95 결정, `tools/mcsim/README.md:124`). 최종 강도는 실장비로 보정합니다(`#151`).
>
> **아직 없는 것은 ②가 아니라 ①의 독립성입니다** — 다른 코드나 해석적 커널의 **산란
> 분포**가 있어야 ①의 순환이 풀립니다. 문헌의 **스칼라 SPR 은 분포를 대신하지 못합니다**
> (`#191` 3번, "독립 산란 분포 확보").
...
> 오히려 낮습니다. **무엇이 그 자리를 정하는지는 확인되지 않았습니다.**
>
> **[정정 2026-09-19, QA-B-134]** 이 문단은 원래 "가장 좁은 커널 항 σ₁ = 0.92–1.21 mm 와
> 자릿수가 같다" 고 적었습니다. **단위 오독이었습니다** — 커널 표의 σ 는 **cm** 이고
> (`virtual_grid.cpp:505` 가 `node.s[i] / pitchCm` 으로 나눕니다), 실제 **σ₁ = 9.19–12.15 mm**
> 입니다. 봉우리는 σ₁ 의 **0.03–0.09배**로, 자릿수가 같지 않습니다.
>
> 가설은 **측정으로 기각됐습니다.** σ₁ 을 k 배로 키우면 봉우리도 따라 밀린다는 예측을
> 검증했는데, σ₁ 이 10.17 → 30.50 mm 로 **3배** 커지는 동안 봉우리는 0.42 → 0.21 → 0.84 →
> 0.42 mm 로 **따라가지 않고 진동**했습니다. 두 번째 후보인 거친 격자 칸 크기도 같은 실험에서
> 탈락했습니다(축소 인자 32 → 98, 칸 4.48 → 13.7 mm, 봉우리 불변).
>
> 봉우리는 늘 **화소 1–8개 자리**에 있습니다. 남은 후보는 **팬텀 경계 기하**(두께 지도가
> 1화소에서 끊김)인데, 그것은 물리 척도가 아니라 **장면 척도**입니다. 따라서
> **"σ₁ 으로 임상적으로 조심할 대역을 예측한다" 는 기대는 성립하지 않습니다.**
>
