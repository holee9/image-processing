...
> **[정정 2026-09-19, #186 — QA-A-133 과 리더가 전 화소 판독으로 확인]**
>
> **아래 표의 `bright01`~`bright06` ↔ "선량 레벨 1"~"6" 대응은 틀렸습니다.**
> 실측 전체 평균(3072×3072 uint16, 헤더 없음)의 오름차순은
> **`03 → 04 → 05 → 06 → 01 → 02`** 입니다:
>
> | 파일 | 전체 평균 | p50 |
> |---|---|---|
> | `dark.raw` | 2503.13 | 2505 |
> | `bright03` | 350.56 | 357 |
> | `bright04` | 867.38 | 885 |
> | `bright05` | 1395.75 | 1424 |
> | `bright06` | 2196.23 | 2241 |
> | `bright01` | 2883.16 | 2955 |
> | `bright02` | 5014.57 | 5129 |
>
> **`bright*` 는 이미 오프셋 보정된 값입니다.** `dark` 는 p0.1=2483 / p50=2505 /
> p99.9=2520 으로 폭 37 ADU 안에 갇힌 전형적 오프셋 대좌인데, `bright` 들의 p0.1 은
> **10~144** 로 2500 근처가 아닙니다 — 보정 전 평탄 영상이라면 암전류 위에 얹혀
> 2500 아래로 내려갈 수 없습니다. **`dark` 를 다시 빼지 마십시오.**
>
> **그리고 선량 값이 이 저장소 어디에도 없습니다.** `fixture.json` 의 `roles` 는
> `bright01`~`06` 이 전부 `"Gain"` 하나이고 **선량·kVp·mAs·노출 시간이 없습니다.**
>
> 그래서 이 데이터셋은 **비선형 LUT 생성에 쓸 수 없습니다.** SRS-CALIB-FUNC-006-EXT
> 6a 의 2단계가 `S_meas` 를 **기준 선량 `D_ref` 와 함께** 요구하고 3단계가
> `S_ideal = G_nominal × D` 인데, **`D` 를 알 길이 없습니다.**
>
> **준위가 몇 개인지 이전의 문제입니다.** 그리고 준위가 등간격이라고 **가정하면
> 측정하려는 선형성을 미리 가정하는 것**이 됩니다.
>
> 미검증: 결함 화소를 빼지 않은 전 화소 평균 / `BPMap.map` 미판독 / 오프셋 보정
> 여부를 대좌 부재로만 판정(생성 이력·장비 설정 미확인) / `dark` 가 `bright` 와 같은
> 세션인지 확인할 메타데이터 없음.
